...
static void num_to_hex(int aa,uint8_t *buffer)
{
    int  i = 0;
    while(buffer[i] != '\0') {
        i++;
...
static int num_to_asc(uint8_t *num,uint8_t *asc,int num_length)
{
    asc[0] = ':';
    asc[1] = '\0';
    int i = 0;
    ...
    for(i = 0;i < num_length;i++) {
        num_to_hex(num[i],asc);
    }
    int asc_length = strlen((char *)asc);
    return asc_length;
}

static uint8_t lrc2(const uint8_t*data,int data_len)
{
    uint8_t lrc = 0;
    int i;
...
int _modbus_asc_check_intergrity(modbus_t *ctx,uint8_t *msg,uint8_t *asc_msg,int asc_msg_length,int msg_length)
{
    uint8_t lrc_calculated;
    uint8_t lrc_received;
    int slave = msg[0];
    ...
    lrc_calculated = lrc2(msg,msg_length-2);
    //crc_calculated = lrc2(asc_msg, asc_msg_length-4);
...
static int asc_to_num(uint8_t *asc_msg,uint8_t *msg,int *start,int msg_length,int end)
{
    int asc_i = *start;
    
    for(asc_i = *start;asc_i < end;asc_i = asc_i+2) {      
        if(asc_msg[asc_i] >='0' && asc_msg[asc_i]<= '9') {
            if(asc_msg[asc_i+1] >= '0' && asc_msg[asc_i+1] <= '9') {
                msg[msg_length] = ((asc_msg[asc_i] - '0') * 16) + (asc_msg[asc_i+1] - '0');
            }else if(asc_msg[asc_i+1] >= 'A' && asc_msg[asc_i+1] <= 'F') {
                msg[msg_length] = ((asc_msg[asc_i] - '0') * 16) + (asc_msg[asc_i+1] - '7');                 
            }
        } else if(asc_msg[asc_i] >= 'A' && asc_msg[asc_i] <= 'F') {
            if(asc_msg[asc_i+1] >='0' && asc_msg[asc_i+1] <= '9') {
                msg[msg_length] = (asc_msg[asc_i] - '7') * 16 + (asc_msg[asc_i+1] - '0');
            }else if(asc_msg[asc_i+1] >='A' && asc_msg[asc_i+1] <= 'F') {
                msg[msg_length] = (asc_msg[asc_i] - '7') * 16 + (asc_msg[asc_i+1] - '7');
            }
        }
        msg_length++;
    }       
...
int _modbus_asc_send_msg_pre(uint8_t *req, int req_length,uint8_t *req_asc)
{
    uint8_t lrc_num;

    lrc_num = lrc2(req, req_length);
    req_length = num_to_asc(req,req_asc,req_length);
    num_to_hex(lrc_num,req_asc);
    req_length = req_length + 2;
    req_asc[req_length++] = '\r';
    req_asc[req_length++] = '\n';
...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length,int asc_msg_len)
{
    int rc;
    int i;
    uint8_t asc_msg[(2 * asc_msg_len)+1];

    //licj: clear garbage data
    modbus_flush(ctx);

    if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
        msg_length = ctx->backend->asc_send_msg_pre(msg, msg_length,asc_msg);
        if (ctx->debug) {
        ...
        if (ctx->print_log)
            ctx->print_log(ctx->slave, 1, (char *)asc_msg, msg_length);
    }else {
        ...
        if (ctx->print_log)
            ctx->print_log(ctx->slave, 1, (char *)msg, msg_length);
    }
    ...
    do {
        if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
            rc = ctx->backend->send(ctx, asc_msg, msg_length);
        }else {
...
int modbus_send_raw_request(modbus_t *ctx, const uint8_t *raw_req, int raw_req_length)
{
    ...
    return send_msg(ctx, req, req_length, MAX_MESSAGE_LENGTH);
}
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    uint8_t start;
    uint8_t num_msg[512];
    int msg_length = 0,num_msg_length = 0,asc_to_int_length = 0;
    ...
    while (length_to_read != 0) {
        ...
        if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
            if(flag == 1){
                length_to_read = 2 * length_to_read;
                rc = ctx->backend->recv(ctx, &start, 1);
                if(start != 0x3A) {
                    if(ctx->debug) {
                        printf("The received message is not ASCLL and The Began to sign is %#x\n",start);
                    }
                    return -1;              
                }
        ...
        if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
            if(msg_length % 2 == 0)
                num_msg_length = asc_to_num(msg, num_msg, &asc_to_int_length, num_msg_length, msg_length);
        }
        ...
        if (length_to_read == 0) {
            ...
            case _STEP_FUNCTION:
                ...
                if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
                    length_to_read = 2 * compute_meta_length_after_function(
                    num_msg[ctx->backend->header_length],
                    msg_type);  
                }else {
            ...
            case _STEP_META:
                if(ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
                    length_to_read = 2 *compute_data_length_after_meta(
                    ctx,num_msg, msg_type);

                    if ((msg_length + length_to_read) > 2 * (int)ctx->backend->max_adu_length) {
                        errno = EMBBADDATA;
    ...
    if (ctx->print_log)
        ctx->print_log(ctx->slave, 0, (char *)msg, msg_length);

    if (ctx->debug) printf("\n");
    if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_ASC) {
        for(j = 0;j < num_msg_length-1;j++){
            if (ctx->debug) printf("%#x ",num_msg[j]);
            msg[j] = num_msg[j];
        }
        if (ctx->debug) printf("\n");

        return ctx->backend->asc_check_intergrity(ctx, num_msg, msg, msg_length, num_msg_length);
    }else{
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    return (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU &&
            slave == MODBUS_BROADCAST_ADDRESS) ? 0 : send_msg(ctx, rsp, rsp_length, MAX_MESSAGE_LENGTH);
}
                           ...
                           unsigned int exception_code)
{
    ...
    if (exception_code < MODBUS_EXCEPTION_MAX) {
        rsp[rsp_length++] = exception_code;
        return send_msg(ctx, rsp, rsp_length, MAX_MESSAGE_LENGTH);
    } else {
        errno = EINVAL;
        return -1;
    }
                          ...
                          int addr, int nb, uint8_t *dest)
{
    ...
    check_msg_length = req_length; 
    ctx->backend->send_msg_pre(req, check_msg_length);

    rc = send_msg(ctx, req, req_length, _MIN_REQ_LENGTH);
    if (rc > 0) {
        int i, temp, bit;
        int pos = 0;
        int offset;
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    rc = send_msg(ctx, req, req_length, MAX_MESSAGE_LENGTH);
    if (rc > 0) {
        uint8_t rsp[MAX_MESSAGE_LENGTH];
...
int modbus_mask_write_register(modbus_t *ctx, int addr, uint16_t and_mask, uint16_t or_mask)
{
    ...
    rc = send_msg(ctx, req, req_length, _MIN_REQ_LENGTH+2);
    if (rc > 0) {
        /* Used by write_bit and write_register */
        uint8_t rsp[MAX_MESSAGE_LENGTH];
...
void modbus_set_debug_log(modbus_t *ctx, void (*print_log) (int slave, int direction, char *msg, int length))
{
    ctx->print_log = print_log;
}
